...
4. **High Performance Computing**:
   ...
   If you are a researcher at MSU, you can use the file race.sb to submit a job to the SLURM system on the HPCC cluster. You can use the development node terminals or the GUI. It will run everything significantly faster than it could locally.
   ```bash
   sbatch race.sb
   ```

   Failed runs will generate .SLURMout files in the same directory with details on the issue. Most likely there is an incorrect file path or it was run in the wrong directory.
   
   The only downside is that the large csv files that are generated by the simulation will not fit under the 50 MB GitHub limit if you do a large run. You can use the compression scripts on the experimental branch to make them smaller, or you can use the GUI and upload them to another cloud platform like Google Drive.
